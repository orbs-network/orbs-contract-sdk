...
func newHarness() *harness {
	return &harness{
		...
		contractName: fmt.Sprintf("MyCounter%d", time.Now().UnixNano()),
	}
}
...
func (h *harness) add(t *testing.T, sender *orbs.OrbsAccount, sum uint64) (*codec.SendTransactionResponse, error) {
	tx, _, err := h.client.CreateTransaction(sender.PublicKey, sender.PrivateKey, h.contractName, "add", sum)
	require.NoError(t, err)
