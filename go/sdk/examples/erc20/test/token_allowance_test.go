...
import (
	...
	"github.com/orbs-network/orbs-contract-sdk/go/sdk/examples/test"
...
func TestAllowance(t *testing.T) {
	...
	h := newHarness()
	deployContract(t, user1)

	t.Run("SimpleAllowance", func(t *testing.T) {
		require.EqualValues(t, 0, allowance(t, user1, user2))

		response, err := approve(t, user1, user2, 3000)
		require.NoError(t, err)
		require.EqualValues(t, "Approval", response.OutputEvents[0].EventName)

		require.True(t, test.Eventually(1*time.Second, func() bool {
			return uint64(3000) == allowance(t, user1, user2)
		}))
	})

	// test relies on setup done in previous
	t.Run("IncreaseAllowance", func(t *testing.T) {
		response, err := increaseAllowance(t, user1, user2, 1000)
		require.NoError(t, err)
		require.EqualValues(t, "Approval", response.OutputEvents[0].EventName)
		require.EqualValues(t, 4000, response.OutputEvents[0].Arguments[2], "event data incorrect")

		require.True(t, test.Eventually(1*time.Second, func() bool {
			return uint64(4000) == allowance(t, user1, user2)
		}))

		response, err = increaseAllowance(t, user1, user2, 18446744073709551615)
		require.NoError(t, err)
	...
	t.Run("DecreaseAllowance", func(t *testing.T) {
		response, err := decreaseAllowance(t, user1, user2, 18446744073709551615)
		require.NoError(t, err)
		...
		response, err = decreaseAllowance(t, user1, user2, 1000)
		require.NoError(t, err)
	...
	t.Run("AllowedTransfer", func(t *testing.T) {
		...
		response, err = transferFrom(t, user2, user1, user3, 2000)
		require.NoError(t, err)
		require.EqualValues(t, "Approval", response.OutputEvents[0].EventName)
