...
func TestTokenTransfer(t *testing.T) {
	...
	require.EqualValues(t, 1000000000000000000, balanceOf(t, user1))

	response, err := transfer(t, user1, user2, 5000)
	require.NoError(t, err)
	require.EqualValues(t, "Transfer", response.OutputEvents[0].EventName)

	require.True(t, test.Eventually(1*time.Second, func() bool {
		return balanceOf(t, user2) == uint64(5000)
	}))

	invalidUser, _ := orbs.CreateAccount()
	invalidUser.Address = "123"
	response, err = transfer(t, user1, invalidUser, 5000)
